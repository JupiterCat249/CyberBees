...
const INIT_PLUG_SCRIPT = """extends "res://addons/gd-plug-plus/plug.gd"

func _plugging():
	# Declare plugins with plug(repo, args)
	# For example, clone from github repo("user/repo_name")
	# plug("imjp94/gd-YAFSM")
	# By default, gd-plug will only install anything
	# from "addons/" directory.
	# Or you can explicitly specify which file/directory
	# to include:
...
func show_syntax() -> void:
	...
	var actions = {
		...
		(
			...
			+ "(installed, added or removed),"
			+ " execute this command whenever in doubts"
		),
	...
	var options = {
		...
		"force":
		(
			"Force gd-plug to overwrite destination"
			+ " files when running install command."
			+ " *WARNING: Check README for more details*"
		),
		"keep-import-file": 'Keep ".import" files generated by plugin, when run uninstall command',
		"keep-import-resource-file":
		(
			'Keep files located in ".import" that'
			+ " generated by plugin, when run"
			+ " uninstall command"
		),
		"help": "Show this help",
...
func show_config_syntax() -> void:
	...
	var arguments = {
		"include":
		(
			"Array of strings that define what files"
			+ " or directory to include."
			+ ' Only "addons/" will be included if omitted'
		),
		...
		"commit":
		(
			"Commit hash string to freeze to,"
			+ " must be full length 40 digits commit-hash,"
			+ " for example, 7a642f90d3fb889..."
		),
		"dev":
		(
			"Boolean to mark the plugin as dev or not,"
			+ " plugin marked as dev will not be installed"
			+ " when production command given"
		),
		"on_updated":
		(
			"Post update hook, a function name"
			+ " declared in plug.gd that will be called"
			+ " whenever the plugin installed/updated"
		)
	}
	...
	for repo_path in _list_repo_dirs(default_plug_dir):
		if not (repo_path in _installed_plugins):
			...
			threadpool.enqueue_task(
				directory_delete_recursively.bind(default_plug_dir.path_join(repo_path))
			)
	threadpool.active = true
...
func download(plugin):
	...
	var result = _GitExecutable.new(plugin.plug_dir).clone(
		plugin.url,
		plugin.plug_dir,
...
func install(plugin):
	...
	if plugin.on_updated:
		if has_method(plugin.on_updated):
			PlugLogger.debug(
				'Execute post-update function "%s" for %s' % [plugin.on_updated, plugin.name]
			)
				...
				for include_key in include:
					if include_key in source:
						...
						if not is_excluded:
							if test:
								if not silent_test:
									PlugLogger.debug("[TEST] Writing to %s" % dest)
							else:
...
func directory_delete_recursively(dir_path, args = {}):
	if str(dir_path).strip_edges().is_empty():
		return null
	var project_root = ProjectSettings.globalize_path("res://")
	var abs_path = str(dir_path)
	if abs_path.begins_with("res://"):
		abs_path = ProjectSettings.globalize_path(abs_path)
	if GitManager._is_path_equal_or_parent(abs_path, project_root):
		PlugLogger.debug(
			"directory_delete_recursively: REFUSED — %s"
			% abs_path
		)
		return null
		...
		while not file_name.is_empty():
			...
			if dir.current_is_dir():
				...
				if not excluded:
					if test:
						if not silent_test:
							PlugLogger.debug("[TEST] Remove file: %s" % source)
					else:
...
func directory_remove_batch(files, args = {}):
	...
	for file in files:
		...
		if file.ends_with(".import"):
			...
			if test:
				if not silent_test:
					PlugLogger.debug("[TEST] Remove file: " + file)
			else:
				if dir.remove(file_name) == OK:
	...
	for dir in dirs.values():
		...
		if test:
			if not silent_test:
				PlugLogger.debug("[TEST] Remove empty directory: %s" % dir.get_current_dir())
		else:
			...
			if d.get_open_error() == OK:
				if test:
					...
					if d.remove(d.get_current_dir()) == OK:
						PlugLogger.debug(
							"Remove empty ancestor directory: %s" % d.get_current_dir()
						)
...
func _remove_import_file(
	...
	if test:
		if not silent_test:
			PlugLogger.debug("[TEST] Remove import file: " + file)
	else:
...
func validate_var_type(obj, var_name, type, type_string):
	...
	if not is_valid:
		PlugLogger.debug(
			'Expected variable "%s" to be %s, given %s' % [var_name, type_string, value]
		)
	return is_valid
...
class _ThreadPool:
	...
	func _execute_task(task):
		...
		if can_execute:
			...
			PlugLogger.debug(
				"Execute task %s.%s() " % [callable.get_object(), callable.get_method()]
			)
		return can_execute
	...
	func _execute(args):
		...
		PlugLogger.debug(
			"Execution finished %s.%s() " % [callable.get_object(), callable.get_method()]
		)
		if all_finished:
