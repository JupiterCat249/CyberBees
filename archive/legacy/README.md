...
### 2.3 `card-system/` —— 已作废的卡牌系统（2026-09-21 整体移入）

- **来源**：项目根 `card-system/`（⚠️ 它是**同仓独立 worktree**，分支 `card-system`，**不是**主分支的文件）
- **状态**：人 2026-09-19 裁定**作废**；2026-09-21 人指示**整体移入本目录**，并完成废弃标注（9 个 `.gd`/`.tscn` → `<path>.deprecated` + 废弃桩，处置提交在该分支）
- **注意**：`.gitignore` 已按新路径忽略（`/archive/legacy/card-system/`）；阅读时请勿把它当作主分支现役代码
