...
## 迭代027（人实测两点）：
##   ① 「幅度太大、频率太低」→ 幅度再缩（7→5 / 13→9）、**段数 13→18**（段数即频率：摆臂更短则抖得更快）
##   ② 「抖动时单位先右移一段、在新位置震、再跳回原位」→ **根因是波形本身漂移**：
##      MOVE_BY 是**累加**位移，旧实现直接交替 ±振幅累加，累加位置序列的**均值不为 0**
##      （实测均值 +7.3px）→ 整段动画都偏在一侧，末帧才跳回。
##      修法（见 battle_anim._register_shake）：先把峰值转成"目标位置"，**减去均值居中**，
##      再转成相邻位移 → 全程围绕**原位**对称震荡，首帧即居中、末帧精确归零。
const SHAKE_FRAMES_UNIT := 36     ## 单位受击抖动：36 帧 ≈ 0.60s @60fps（18 段 × 2 帧）
