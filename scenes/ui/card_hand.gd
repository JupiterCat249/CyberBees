...
## 惰性取节点（不用 @onready —— 因为 bind() 可能在 _ready() 之前/
## 之中被父节点调用，而 @onready 变量要等 _ready() 结束才赋值 → 会拿到 null）
func _img() -> TextureRect: return $Artwork/ArtPlane/Image
func _badge() -> Label: return $BadgeImage/Value
func _body_panel() -> Panel: return $Body

func _ready() -> void:
	mouse_filter = Control.MOUSE_FILTER_STOP
	mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
	set_meta("image_base_offset", Vector2(_img().offset_left, _img().offset_top))
	set_meta("image_base_scale", _img().scale)
...
func set_image_offset(new_offset: Vector2, new_scale: Vector2 = Vector2.ZERO) -> void:
	...
	if sz == Vector2.ZERO:
		sz = Vector2(_img().offset_right - _img().offset_left, _img().offset_bottom - _img().offset_top)
		set_meta("image_base_size", sz)
	_img().offset_left = new_offset.x
	_img().offset_top = new_offset.y
	_img().offset_right = new_offset.x + sz.x
	_img().offset_bottom = new_offset.y + sz.y
	if new_scale != Vector2.ZERO:
		_img().scale = new_scale
